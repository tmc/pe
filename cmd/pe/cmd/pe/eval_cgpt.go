...
// generateStableHash creates a stable, numeric hash from a string
func generateStableHash(s string) uint64 {
...
func evaluateWithCGPTOptions(config map[string]interface{}, dryRun bool) (map[string]interface{}, error) {
	...
	// Initialize CGPT provider with the default model
	provider := cgpt.DefaultProvider()
	
	// Create a unique evalId for this run
	evalId := fmt.Sprintf("eval-cgpt-%s", time.Now().Format("20060102T150405"))
	...
	for i, prompt := range prompts {
		...
		for j, test := range tests {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error evaluating prompt: %v\n", err)
				continue
			}
			...
			detailedResults = append(detailedResults, map[string]interface{}{
				...
				"provider": map[string]interface{}{
					"id":    fmt.Sprintf("%s:%s", provider.Backend, provider.Model),
					"label": "",
				},
