...
func promptInitCmd() *cobra.Command {
	var (
		provider     string
		system       string
		withDefaults bool
...
// promptEditCmd edits prompt file metadata and defaults
  ...
  pe prompt info my.prompt --verbose`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if jsonOutput {
				...
				if vars, ok := info["variables"].([]string); ok && len(vars) > 0 {
					fmt.Printf("Variables: %s\n", strings.Join(vars, ", "))
				}

				if defaults, ok := info["defaults"].(map[string]string); ok && len(defaults) > 0 {
				...
				if sections, ok := info["sections"].([]string); ok && len(sections) > 0 {
					fmt.Printf("Sections: %s\n", strings.Join(sections, ", "))
				}

				if verbose {
					if tests, ok := info["test_count"].(int); ok && tests > 0 {
						fmt.Printf("Tests: %d\n", tests)
					}
					if variants, ok := info["variants"].([]string); ok && len(variants) > 0 {
...
// analyzePromptFile analyzes a prompt file and returns metadata
func analyzePromptFile(content string) map[string]interface{} {
	...
	if len(lines) > 0 && strings.HasPrefix(lines[0], "#!") {
		...
		if strings.Contains(lines[0], "--provider=") {
			...
			for _, part := range parts {
				if strings.HasPrefix(part, "--provider=") {
					info["provider"] = strings.TrimPrefix(part, "--provider=")
				}
			}
		}
	} else {
	...
	// Find variables using template syntax
	...
	for _, line := range lines {
		...
		if _, err := tmpl.Parse(line); err == nil {
			...
			for _, match := range findVariables(line) {
				varMap[match] = true
			}
		}
	}
	...
	for _, line := range lines {
		...
		} else if currentSection == "defaults" && strings.Contains(line, ":") {
			parts := strings.SplitN(line, ":", 2)
			if len(parts) == 2 {
				key := strings.TrimSpace(parts[0])
				value := strings.TrimSpace(parts[1])
		...
		} else if currentSection == "variants" && strings.TrimSpace(line) != "" && !strings.HasPrefix(line, " ") {
			if strings.Contains(line, ":") {
				variantName := strings.TrimSuffix(line, ":")
				variants = append(variants, variantName)
			}
		} else if currentSection == "tests" && strings.HasPrefix(strings.TrimSpace(line), "- name:") {
			testCount++
		}
	}
	...
	if testCount > 0 {
		info["test_count"] = testCount
	}

	return info
}

// promptTidyCmd cleans up and validates prompt files
func promptTidyCmd() *cobra.Command {
	var (
		removeUnused bool
		validate     bool
	)
...
func tidyPromptFile(filename string, removeUnused, validate bool) error {
	...
	for _, line := range lines {
		for _, v := range findVariables(line) {
			usedVars[v] = true
		}
	}
	...
	if validate {
		mainContent := extractMainContent(lines)
	...
	for i := 0; i < len(text); i++ {
		if i+3 < len(text) && text[i:i+3] == "{{." {
			...
			if end > 0 {
				...
				if varName != "" && !contains(vars, varName) {
