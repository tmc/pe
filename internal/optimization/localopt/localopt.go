...
// Variant is one prompt candidate in a local optimization run.
type Variant struct {
	Prompt   string
	Source   string
...
func (f ScorerFunc) Score(ctx context.Context, variant Variant) (Score, error) {
	if f == nil {
		return Score{}, fmt.Errorf("local optimizer: nil scorer func")
	}
...
func (f RefinerFunc) Refine(ctx context.Context, current Variant) ([]Variant, error) {
	if f == nil {
		return nil, fmt.Errorf("local optimizer: nil refiner func")
	}
	return f(ctx, current)
}

// Config controls local optimization.
type Config struct {
	MaxRounds      int
...
// Result records the full local optimization trajectory.
type Result struct {
	Initial   Step
	Best      Step
	Steps     []Step
...
func New(scorer Scorer, refiner Refiner, config Config) (*Optimizer, error) {
	if scorer == nil {
		return nil, fmt.Errorf("local optimizer: nil scorer")
	}
	if refiner == nil {
		return nil, fmt.Errorf("local optimizer: nil refiner")
	}
	if config.MaxRounds < 0 {
		return nil, fmt.Errorf("local optimizer: negative max rounds")
	}
	if config.MaxRounds == 0 {
		config.MaxRounds = 1
	}
	if config.MinImprovement < 0 {
		return nil, fmt.Errorf("local optimizer: negative min improvement")
	}
	return &Optimizer{
		scorer:  scorer,
		refiner: refiner,
