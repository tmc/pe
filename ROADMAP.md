...
5. Connect `pe.mod` capability, placement, and policy blocks to static
   ...
   DONE current pass: `pe extract --output` now enforces `tools deny write`
   before writing extracted content files, while stdout extracted output
   remains read-only. Remaining work: runtime enforcement for other tool
   classes and remaining write surfaces.
   DONE current pass: `pe evolve --output` now enforces `tools deny write`
   before writing evolution result files, while terminal evolution output
   remains read-only. Remaining work: runtime enforcement for other tool
   classes and remaining write surfaces.
   DONE current pass: security report output files now enforce
   ...
   Remaining work: runtime enforcement for other tool classes and remaining
   write surfaces.
   DONE current pass: `pe structured` file outputs now enforce
   `tools deny write` through a shared output helper, while terminal structured
   output remains read-only. Remaining work: runtime enforcement for other
   tool classes and remaining write surfaces.
   DONE current pass: template file outputs now enforce `tools deny write`
